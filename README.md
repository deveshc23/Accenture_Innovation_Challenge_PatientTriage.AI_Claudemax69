...
```
                 ...
                 │                    │                    │
                 ▼                    ▼                    ▼
        textFlags(complaint)   scoreVitals(pt)      history / obs chips
        bilingual match +      NEWS2-style EWS,
        negation handling      patient's own age band
                 │                    │                    │
                 └────────────────────┼────────────────────┘
                                      │
        ┌─────────────┬──────────────┼──────────────┬─────────────┐
   ...
   │ life-   │   │ red-flag│   │ risk    │   │ vital-  │          │
   │ saving  │   │ patterns│   │modifiers│   │ sign EWS│          │
   └────┬────┘   └────┬────┘   └────┬────┘   └────┬────┘          │
        │             │             │             │               │
        └──────────────────┬────────────────────────┘             │
                            ▼                                      │
                  ...
                  the model can never see past this line           │
                            │                                      │
                            │            ┌─────────────────────────┘
                            │            ▼
                            │   riskModel(pt) → probability p
                            │   monotone gradient-boosted surface,
                            │   band-relative deviations only
                            │            │
                            └─────┬──────┘
                                  ▼
                    final = min(floor, model.acuity)
                    model may TIGHTEN only, never loosen
                                  │
                                  ▼
                    confidence(pt, layers, model)
                    completeness · agreement · margin
                                  │
                          ┌───────┴────────┐
                          ▼                ▼
                  confidence ≥ 0.50   confidence < 0.50
                  AND complete enough  or incomplete data
                          │                │
                          ▼                ▼
                 final acuity          held at ESI‑3
                 (RED/YELLOW/GREEN)    → clinician rapid lane
                          │                │
                          └───────┬────────┘
                                  ▼
                     Recommendation + execution trace
                     shown to the clinician, with reasons
                                  │
                          ┌───────┴────────┐
                          ▼                ▼
                      Accept              Override
                  (as recommended)   (new level + required
                                       reason code + evidence)
                          │                │
                          └───────┬────────┘
                                  ▼
                    Written to the hash-chained audit log
                    attributed to the clinician's licence
```
...
**Safe-wait / re-score flow, per patient:**

```
                    ┌───────────────────────────┐
                    │   Patient enters the       │
                    │   waiting room, level set   │
                    └──────────────┬──────────────┘
                                   │
                                   ▼
                    ┌───────────────────────────┐
                    │  Safe-wait timer starts     │
                    │  RED 20m · YELLOW 2h ·      │
                    │  GREEN 5h  (or a shortened   │
                    │  window — see below)         │
                    └──────────────┬──────────────┘
                                   │  window elapses
                                   ▼
                    ┌───────────────────────────┐
                    │  Nurse retakes vitals and    │
                    │  the engine re-scores them   │
                    └──────────────┬──────────────┘
                                   │
                    ┌──────────────┴──────────────┐
                    ▼                              ▼
        No significant change              Crossed a colour band,
        (same band, EWS steady)             or EWS jumped ≥ 3
                    │                              │
                    ▼                              ▼
        Routine re-check logged          Escalated + re-ranked +
        (REMEASURE) — patient            flagged in the UI
        keeps its normal window          (RETRIAGE_ESCALATION)
                    │                              │
                    │                  ┌────────────┴────────────┐
                    │                  ▼                          ▼
                    │            Admitted /                Still in the room,
                    │            removed from               not admitted
                    │            the queue                        │
                    │                  │                          ▼
                    │                  │            Deterioration noted
                    │                  │            (DETERIORATION_NOTED)
                    │                  │                          │
                    │                  │                          ▼
                    │                  │            Next safe-wait window
                    │                  │            shortened for this patient:
                    │                  │            1st note  → stage-2 value
                    │                  │            2nd note  → holds
                    │                  │            3rd+ note → × 0.9 compounding
                    │                  │                          │
                    ▼                  ▼                          ▼
              back to "timer starts" with the (possibly shortened) window
                          — every step written to the audit chain —
```
